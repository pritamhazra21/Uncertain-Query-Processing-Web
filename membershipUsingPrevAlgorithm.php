...
$result = mysqli_query($conn, "SELECT AVG($data_col) AS avg FROM project_db;");
$row = mysqli_fetch_assoc($result);
$avg = $row['avg'];
...
while ($row = mysqli_fetch_assoc($result)) {
    $id = $row['id'];
    $membershipValue = 1 - (abs($row[$data_col]- $fuzzy_data) / $avg);

    $update_sql = "UPDATE project_db SET $membership_col = $membershipValue, $fuzzy_col = $fuzzy_data WHERE id = $id";
    mysqli_query($conn, $update_sql);
}

echo '[{"status": "membership value on prev algorithm generated"}]';
